...
object WhiteboardStateReducer {
    ...
    fun updateText(state: WhiteboardState, index: Int, newValue: String, color: ColorHex, sizeSp: Float): WhiteboardState {
        val value = newValue.trim()
        if (value.isEmpty() || index !in state.texts.indices) return state
        val updated = state.texts.toMutableList()
        updated[index] = updated[index].copy(text = value, color = color, sizeSp = sizeSp.coerceIn(12f, 96f))
        return state.copy(texts = updated)
    }
    ...
    fun eraseTextRange(state: WhiteboardState, index: Int, startInclusive: Int, endExclusive: Int): WhiteboardState {
        ...
        val updated = state.texts.toMutableList()
        if (updatedText.isEmpty()) updated.removeAt(index) else updated[index] = original.copy(text = updatedText)
        return state.copy(texts = updated)
    }
    ...
    fun moveShape(state: WhiteboardState, index: Int, dx: Float, dy: Float): WhiteboardState {
        ...
        shapes[index] = when (val old = shapes[index]) {
            is ShapeEntity.Rectangle -> old.copy(rect = Rect(old.rect.left + dx, old.rect.top + dy, old.rect.right + dx, old.rect.bottom + dy))
            is ShapeEntity.Circle -> old.copy(center = Point(old.center.x + dx, old.center.y + dy))
            is ShapeEntity.Line -> old.copy(start = Point(old.start.x + dx, old.start.y + dy), end = Point(old.end.x + dx, old.end.y + dy))
            is ShapeEntity.Polygon -> old.copy(bounds = Rect(old.bounds.left + dx, old.bounds.top + dy, old.bounds.right + dx, old.bounds.bottom + dy))
        }
    ...
    fun resizeShape(state: WhiteboardState, index: Int, newBounds: Rect): WhiteboardState {
        ...
        shapes[index] = when (val old = shapes[index]) {
            ...
            is ShapeEntity.Circle -> old.copy(center = Point(newBounds.centerX(), newBounds.centerY()), radius = min(newBounds.width(), newBounds.height()) / 2f)
            is ShapeEntity.Line -> old.copy(start = Point(newBounds.left, newBounds.top), end = Point(newBounds.right, newBounds.bottom))
    ...
    fun eraseAt(state: WhiteboardState, point: Point, radiusPx: Float): WhiteboardState {
        return state.copy(
            strokes = state.strokes.flatMap { it.erase(point, radiusPx) },
            shapes = state.shapes.filterNot { it.hitTest(point, radiusPx) },
            texts = state.texts.filterNot { it.hitTest(point, radiusPx) }
        )
    }
}
